...
class FoundPHP_ftp{
	...
	*/
	function put($file_name='',$save_name=''){
		...
		if ($this->ssh==1){
			$sftpStream = @fopen('ssh2.sftp://'.$this->links.'/'.$save_name, 'w');
			if (!$sftpStream) {
	...
	*/
	function get($file_name='',$save_name=''){
		...
		if ($this->ssh==1){
			...
			$remote_file = @fopen ('ssh2.sftp://'.$this->links.'/'.$file_name); // 远程下载文件，二进制模式
			if ($remote_file) {
				$local_file = fopen ($save_name, "wb"); // 远在文件文件
				if ($local_file){
				while (!feof($remote_file)) { // 判断附件写入是否完整
					fwrite($local_file, fread($remote_file, 1024 * 8), 1024 * 8); // 没有写完就继续
				}
				}
			}
			@fclose($remote_file);		// 关闭远程文件
			@fclose($newf);				// 关闭本地文件
	...
	function del($file_name=''){
		if ($this->ssh==1){
			if (ssh2_sftp_unlink($this->links, $file_name)) {
				...
				return json(1,array('file_name'=>$file_name));
			} else {
	...
	function mk_dir($name=''){
		if ($this->ssh==1){
			if (ssh2_sftp_mkdir($this->links, $name,0777)) {
				...
				return json(1,array('dir_name'=>$name));
			} else {
	...
	function name($old_name='',$new_name=''){
		...
		if ($this->ssh==1){
			if (@ssh2_sftp_rename($this->links, $old_name,$new_name)) {
				...
				return json(1,array('new_name'=>$new_name));
			} else {
